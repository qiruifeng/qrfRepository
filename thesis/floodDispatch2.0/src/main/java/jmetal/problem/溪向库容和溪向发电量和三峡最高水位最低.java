...
import static until.CalculateUtil.getSum;
import static until.Msjg.riverevolustion;

public class 溪向库容和溪向发电量和三峡最高水位最低 extends AbstractDoubleProblem {
    ...
    public 溪向库容和溪向发电量和三峡最高水位最低(int[] T, int stationNum, double[] levelStart) {
        this.T = T;
...
//                    Math.min(600.0, 560.0 + (i - xNum * 2 + 1) * 5);
    ...
    public void evaluate(DoubleSolution solution) {
        ...
        double target3 = getDoubleArrMax(Z_sx);
    ...
    private void initData() {
        ...
        switch (this.stationNum) {
            ...
            case 3: {
                double[][] inQFromXLDOrigin = EasyExcelUtil.readTable("入库数据", 4, T[0], T[1]);
                double[][] inQFromSXOrigin = EasyExcelUtil.readTable("入库数据", 4, T[0], T[1]);
