...
public class 固定预留库容_下泄流量和发电量 extends AbstractDoubleProblem implements ConstrainedProblem<DoubleSolution> {
...
//    private int[] period;
    ...
    public 固定预留库容_下泄流量和发电量(int[] T, int stationNum, double[] levelStart, double reserveStorage) {
        ...
        if (stationNum == 1) {
            ...
            setNumberOfObjectives(2);
            setNumberOfConstraints(T[1] - T[0] + 1);
        }
...
//            System.out.println("powerXJB:"+powerXJB/100000000);
            ...
            double target2 = getPingFangHe(QoutXJB);//下泄流量平方和最小
            double target3 = 1 / getDoubleArrMin(V_left);//预留库容最大
            double q = getDoubleArrMax(QoutXJB);//向家坝出库流量最大值
            double V_left_min = getDoubleArrMin(V_left);
            solution.setObjective(0, target1);
            solution.setObjective(1, target2);
            solution.setAttribute("Qmax", q);
            solution.setAttribute("n", n);
            solution.setAttribute("V_left_min", V_left_min);
    ...
    private void initData() {
        ...
        switch (this.stationNum) {
            case 1: {
                double[][] inQFromSXOrigin = EasyExcelUtil.readTable("入库数据", 2, T[0], T[1]);
